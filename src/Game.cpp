...
bool Game::playFromHand(size_t index, int targetSystemId, bool targetHasCivilization) {
  ...
  if (cardNeedsTarget(*cardDef)) {
    ...
    if (!rules_.isValidSystemTarget(targetSystemId, *cardDef)) {
      std::cout << "Target system invalid or out of range.\n";
      return false;
    }
  } else {
    targetSystemId = -1;
  ...
  for (const auto& message : network_.receiveMessages()) {
    if (message.payload.rfind("ACT ", 0) == 0) {
      ...
      if (playerId >= 0 && playerId < static_cast<int>(pendingActions_.size())) {
        ...
        if (action.type == ActionType::Play && cardDef && cardNeedsTarget(*cardDef) &&
            action.targetSystemId < 0) {
          network_.sendMessage("Target required for " + cardDef->title + ".");
          pendingReady_[playerId] = false;
          continue;
        }
        if (action.type == ActionType::Play && cardDef && cardNeedsTarget(*cardDef) &&
            !rules_.isValidSystemTarget(action.targetSystemId, *cardDef)) {
          network_.sendMessage("Invalid target system for " + cardDef->title + ".");
          pendingReady_[playerId] = false;
          continue;
        }
