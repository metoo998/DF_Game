...
bool GameRules::isValidSystemTarget(int systemId) const {
...
bool GameRules::isValidSystemTarget(int systemId, const CardDefinition& card) const {
  if (!isValidSystemTarget(systemId)) {
    return false;
  }
  if (card.category != CardCategory::Strike) {
    return true;
  }
  const auto& target = systems_[systemId];
  if (target.isDimensionalized) {
    return false;
  }
  if (card.id == 12 && !target.starExists) {
    return false;
  }
  return true;
}
...
void GameRules::queueProjectile(const std::string& cardName, int ownerId, int targetSystemId,
                                int level, int cardId, int expeditionEnergy, int cardCost) {
  projectiles_.push_back(
      {cardName, 1, ownerId, targetSystemId, level, cardId, expeditionEnergy, cardCost});
...
void GameRules::addPlayer(int playerId) {
  ...
  if (playerId >= static_cast<int>(visibilityCache_.size())) {
    visibilityCache_.resize(playerId + 1);
  }
}
...
void GameRules::resolveStrikeProjectile(const Projectile& projectile) {
  ...
  if (target.isDimensionalized) {
    int refund = projectile.expeditionEnergy > 0 ? projectile.expeditionEnergy : projectile.cardCost;
    players_[projectile.ownerId].energy += refund;
    std::cout << "[Rules] Strike invalidated (2D target). Refunded energy: " << refund << "\n";
    return;
  }
  if (projectile.cardId == 12 && !target.starExists) {
    int refund = projectile.expeditionEnergy > 0 ? projectile.expeditionEnergy : projectile.cardCost;
    players_[projectile.ownerId].energy += refund;
    std::cout << "[Rules] Light strike invalidated (no star). Refunded energy: " << refund << "\n";
    return;
  }
...
std::vector<ObservedSystemState> GameRules::buildObservationSnapshot(
  ...
  if (playerId < 0 || playerId >= static_cast<int>(visibilityCache_.size())) {
    return snapshot;
  }
  for (const auto& entry : visibilityCache_[playerId]) {
    snapshot.push_back({entry.systemId, prefix + entry.starStatus, entry.ageInRounds});
  }
  return snapshot;
}
...
void GameRules::updateVisibility() {
  for (size_t playerId = 0; playerId < playerSystems_.size(); ++playerId) {
    visibilityCache_[playerId].clear();
    ...
    for (size_t systemId = 0; systemId < systems_.size(); ++systemId) {
      ...
      std::string status = systemState.isDimensionalized
                               ? "2D collapse"
                               : (systemState.starExists ? "Star present" : "Star absent");
      int age = distance == 0 ? 0 : distance - 1;
      visibilityCache_[playerId].push_back({static_cast<int>(systemId), status, age});
    }
  }
}
int GameRules::shortestDistance(int startSystemId, int targetSystemId) const {
  if (startSystemId == targetSystemId) {
    return 0;
  }
  if (startSystemId < 0 || targetSystemId < 0 ||
      startSystemId >= static_cast<int>(systems_.size()) ||
      targetSystemId >= static_cast<int>(systems_.size())) {
    return -1;
  }
