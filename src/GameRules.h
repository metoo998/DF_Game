...
public:
  ...
  bool isValidSystemTarget(int systemId) const;
  bool isValidSystemTarget(int systemId, const CardDefinition& card) const;
  ...
  void queueProjectile(const std::string& cardName, int ownerId, int targetSystemId, int level,
                       int cardId, int expeditionEnergy, int cardCost);
...
private:
  ...
  std::vector<std::vector<ObservedSystemState>> visibilityCache_{};
